...
def outPutFile(devPred, epochNum, idxToTag):
    #outPath = "try1/devEpoch" + str(epochNum + 1) +".out"
    outPath = devOutPath
    devPath = mainPath + "dev"
...
def outputTest(modelPath, testPath, outPath):
    ...
    if torch.cuda.is_available():
        ...
        model = LSTMNER(vocab_size, embedding_dim, hidden_dim, output_dim, final_dim, pad_idx, drop_out_prob)
...
class LSTMNER(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim, output_dim, final_dim, pad_idx, drop_out_prob):
        super().__init__()
        #self.embedding = nn.Embedding(vocab_size, embedding_dim, padding_idx=pad_idx)
        self.embedding = nn.Embedding(vocab_size, embedding_dim)
        self.lstm = nn.LSTM(embedding_dim + 1, hidden_dim, batch_first=True, bidirectional=True)
...
def load_model(modelPath):
    ...
    print("dev1.out has been generated successfully!")
...
def train_dev():
    ...
    for epoch in range(n_epochs):
        ...
        if epoch > 7:
            ...
            torch.save(model.state_dict(), 'task1Model/T1epoch' + str(epoch + 1) + 'model.pt')
...
#outputTest(modelPath, testPath, 'test1.out')